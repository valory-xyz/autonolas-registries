...
describe('Simple collectible', function () {
    ...
    it('nft example', async function mint_nft() {
        ...
        const { provider, program, storage } = await loadContract('SimpleCollectible', [mint, metadata_authority.publicKey, baseURI]);

        const number = new BN(1);
        const result = await program.methods.tokenUri(number)
