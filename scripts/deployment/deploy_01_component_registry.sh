...
componentRegistrySymbol=$(jq -r '.componentRegistrySymbol' $globals)
baseURI=$(jq -r '.baseURI' $globals)

contractName="ComponentRegistry"
contractPath="contracts/$contractName.sol:$contractName"
constructorArgs="$componentRegistryName $componentRegistrySymbol $baseURI"
...
# Get output length
outputLength=${#componentRegistryAddress}
...
echo "$(jq '. += {"componentRegistryAddress":"'$componentRegistryAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$componentRegistryAddress $contractPath --constructor-args $(cast abi-encode "constructor(string,string,string)" "$componentRegistryName" "$componentRegistrySymbol" "$baseURI")"
...
echo "$contractName deployed at: $componentRegistryAddress"