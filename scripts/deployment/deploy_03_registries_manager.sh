...
constructorArgs="$componentRegistryAddress $agentRegistryAddress"
...
registriesManagerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#registriesManagerAddress}
...
echo "$(jq '. += {"registriesManagerAddress":"'$registriesManagerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$registriesManagerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address)" $constructorArgs)"
...
echo "$contractName deployed at: $registriesManagerAddress"