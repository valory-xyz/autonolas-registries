...
serviceRegistryTokenUtilityAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#serviceRegistryTokenUtilityAddress}
...
echo "$(jq '. += {"serviceRegistryTokenUtilityAddress":"'$serviceRegistryTokenUtilityAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$serviceRegistryTokenUtilityAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
...
echo "$contractName deployed at: $serviceRegistryTokenUtilityAddress"