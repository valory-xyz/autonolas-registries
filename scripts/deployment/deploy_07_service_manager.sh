...
serviceManagerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#serviceManagerAddress}
...
echo "$(jq '. += {"serviceManagerAddress":"'$serviceManagerAddress'"}' $globals)" > $globals
...
echo "$contractName deployed at: $serviceManagerAddress"