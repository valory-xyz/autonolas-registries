...
contractName="ServiceManagerProxy"
contractPath="contracts/$contractName.sol:$contractName"
constructorArgs="$serviceManagerAddress $proxyData"
...
serviceManagerProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#serviceManagerProxyAddress}
...
echo "$(jq '. += {"serviceManagerProxyAddress":"'$serviceManagerProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$serviceManagerProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "$contractName deployed at: $serviceManagerProxyAddress"