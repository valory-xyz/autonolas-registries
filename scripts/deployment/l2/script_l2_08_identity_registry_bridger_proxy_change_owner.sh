...
# chain's governance control point, by calling changeOwner(bridgeMediator). On
...
echo "${green}Change IdentityRegistryBridgerProxy owner: $currentOwner -> $newOwnerAddress${reset}"
castArgs="$identityRegistryBridgerProxyAddress changeOwner(address) $newOwnerAddress"
echo $castArgs
