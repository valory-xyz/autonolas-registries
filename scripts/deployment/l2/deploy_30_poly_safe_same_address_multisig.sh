...
polySafeSameAddressMultisigAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#polySafeSameAddressMultisigAddress}
...
echo "$(jq '. += {"polySafeSameAddressMultisigAddress":"'$polySafeSameAddressMultisigAddress'"}' $globals)" > $globals
...
echo "$contractName deployed at: $polySafeSameAddressMultisigAddress"