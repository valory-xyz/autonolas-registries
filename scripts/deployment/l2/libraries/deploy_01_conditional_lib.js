...
async function main() {
    ...
    console.log("1. EOA to deploy ConditionalLib");
    const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, "gwei");
    const ConditionalLib = await ethers.getContractFactory("ConditionalLib");
    console.log("You are signing the following transaction: ConditionalLib.connect(EOA).deploy()");
    const conditionalLib = await ConditionalLib.connect(EOA).deploy({ gasPrice });
    const result = await conditionalLib.deployed();

    // Transaction details
    console.log("Contract deployment: ServiceRegistryL2");
    console.log("Contract address:", conditionalLib.address);
    console.log("Transaction:", result.deployTransaction.hash);
    // Wait half a minute for the transaction completion
    await new Promise(r => setTimeout(r, 30000));

    // Writing updated parameters back to the JSON file
    parsedData.conditionalLibAddress = conditionalLib.address;
    fs.writeFileSync(globalsFile, JSON.stringify(parsedData));

    // Contract verification
    if (parsedData.contractVerification) {
        const execSync = require("child_process").execSync;
        execSync("npx hardhat verify --network " + providerName + " " + conditionalLib.address, { encoding: "utf-8" });
    }
}
