...
safeAndDepositWalletCreatorAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#safeAndDepositWalletCreatorAddress}
...
echo "$(jq '. += {"safeAndDepositWalletCreatorAddress":"'$safeAndDepositWalletCreatorAddress'"}' $globals)" > $globals
...
echo "$contractName deployed at: $safeAndDepositWalletCreatorAddress"
