...
hashCheckpointAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#hashCheckpointAddress}
...
echo "$(jq '. += {"hashCheckpointAddress":"'$hashCheckpointAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$hashCheckpointAddress $contractPath --constructor-args $(cast abi-encode "constructor(string)" "$baseURI")"
...
echo "$contractName deployed at: $hashCheckpointAddress"