...
operatorWhitelistAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#operatorWhitelistAddress}
...
echo "$(jq '. += {"operatorWhitelistAddress":"'$operatorWhitelistAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$operatorWhitelistAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
...
echo "$contractName deployed at: $operatorWhitelistAddress"