...
agentRegistrySymbol=$(jq -r '.agentRegistrySymbol' $globals)
...
constructorArgs="$agentRegistryName $agentRegistrySymbol $baseURI $componentRegistryAddress"
...
agentRegistryAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#agentRegistryAddress}
...
echo "$(jq '. += {"agentRegistryAddress":"'$agentRegistryAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$agentRegistryAddress $contractPath --constructor-args $(cast abi-encode "constructor(string,string,string,address)" "$agentRegistryName" "$agentRegistrySymbol" "$baseURI" "$componentRegistryAddress")"
...
echo "$contractName deployed at: $agentRegistryAddress"